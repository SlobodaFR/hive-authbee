import { ValidateVerifyTokenParams, ValidateVerifyTokenResult } from "./types";
...
export class ValidateVerifyTokenUseCase {
  ...
  async execute({
    token,
  }: ValidateVerifyTokenParams): Promise<ValidateVerifyTokenResult> {
    ...
    const savedToken = await this.tokenRepository.renewForUserAndType(
      tokenData.user.id,
      "ACCESS",
    );
