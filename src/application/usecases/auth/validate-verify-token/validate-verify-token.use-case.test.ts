...
describe("ValidateVerifyTokenParams", () => {
  ...
  test("throw error on unknown token", async () => {
    ...
    await expect(
      validateVerifyTokenUseCase.execute({ token: "123456" }),
    ).rejects.toThrow(
      new Error(`validateVerifyToken: ${ERRORS.UNKNOWN_TOKEN}`),
    );
  });
  ...
  test("throw error on unknown token user", async () => {
    ...
    await expect(
      ...
      new Error(`validateVerifyToken: ${ERRORS.NOT_LINKED_TO_USER}`),
    );
  });
  ...
  test("create access token on valid token", async () => {
    ...
    await validateVerifyTokenUseCase.execute({ token: "123456" });
