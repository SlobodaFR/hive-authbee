...
describe("GetVerifyToken", () => {
  ...
  const getVerifyTokenUseCase = new GetVerifyTokenUseCase(
    logger,
    emailValidator,
    userRepository,
    tokenRepository,
  ...
  test("throw error on invalid email", async () => {
    ...
    await expect(getVerifyTokenUseCase.execute({ email: "" })).rejects.toThrow(
      new Error(`getVerifyToken: ${ERRORS.VALID_EMAIL_REQUIRED}`),
    );
    expect(emailValidator.isValid).toHaveBeenCalledWith("");
    await expect(
      getVerifyTokenUseCase.execute({ email: "foobar" }),
    ).rejects.toThrow(
      new Error(`getVerifyToken: ${ERRORS.VALID_EMAIL_REQUIRED}`),
    );
  ...
  test("upsert user on valid email", async () => {
    await getVerifyTokenUseCase.execute({ email });
  ...
  test("create verify token on valid email", async () => {
    ...
    expect(tokenRepository.renewForUserAndType).toHaveBeenCalledWith(
      "1",
      "VERIFY",
    );
  });
  ...
  test("return token on valid email", async () => {
    const result = await getVerifyTokenUseCase.execute({ email });
