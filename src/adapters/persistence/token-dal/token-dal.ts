...
export class TokenDAL implements TokenRepository {
    ...
    private mapDatabaseObjectToDomainObject({id, tokenType, value, expiresAt, updatedAt, userId}: DbToken): Token {
        return {
            id,
    ...
    async renewForUserAndType(userId: string, tokenType: TokenType): Promise<Token> {
        ...
        return this.mapDatabaseObjectToDomainObject(await this.prismaClient.token.create({
            data: {
                expiresAt,
                userId,
                tokenType,
    ...
    async getByValue(value: string): Promise<Token | null> {
        return this.mapDatabaseObjectToDomainObject(await this.prismaClient.token.findFirstOrThrow({
            where: {
                value
            },
            select: {
